...
public class SplitPage {
	...
	public void setPageRows(int pageRows) {
		...
		this.totalPages = (this.totalRows % this.pageRows == 0) ? this.totalRows
				/ this.pageRows
	...
	public int confirmPage(String flag) {
		...
		if (flag != null) {// flag只可能是下面值之一
			if (flag.equals(SplitPage.FIRSTPAGE)) {
				...
				newPage = (this.totalPages == this.currentPage) ? this.currentPage
						: this.currentPage + 1;
			} else if (flag.equals(SplitPage.PREVIOUSEPAGE)) {
				// 第一个页面和当前页面相等吗,如果是那么页面编号不往前走,否则页面编号减一
				newPage = (this.firstPage == this.currentPage) ? this.currentPage
						: this.currentPage - 1;
			} else {// 否则是一个数字字符串
