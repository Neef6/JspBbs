    ...
    <form name="userUpd" action="userAction?cmd=upd" method="post">
    	...
    	<table>
    		...
    		<tr>
    			<td>用户状态:</td>
    			<td width="40%">
    				<select name="bbsState.stateId">
    					<c:forEach items="${listState}" var="state" varStatus="status">
							<c:if test="${data.bbsState.stateId==state.stateId}">
								<option  value=${ state.stateId} selected>${state.stateDes }</option>
							</c:if>
							<c:if test="${data.bbsState.stateId!=state.stateId}">
								<option  value=${ state.stateId}>${state.stateDes }</option>
							</c:if>						
    					</c:forEach>
    				</select>
    			</td><td></td>
    		...
    		<tr>
    			<td>用户等级:</td>
    			<td width="40%">
    				<select name="bbsRole.roleId">
    					<c:forEach items="${listRole}" var="role" varStatus="status">
							<c:if test="${data.bbsRole.roleId==role.roleId}">
								<option  value=${ role.roleId} selected>${role.roleDes }</option>
							</c:if>
							<c:if test="${data.bbsRole.roleId!=role.roleId}">
								<option  value=${ role.roleId}>${role.roleDes }</option>
							</c:if>
    					</c:forEach>
    				</select>
    			</td><td></td>
